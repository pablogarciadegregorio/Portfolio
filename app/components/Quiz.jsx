...
export default function Quiz({ quizClose }) {
  const questions = [
...

    {
      ...
      answerOptions: [
        ...
        {
          answerText: "Personas con discapacidad intelectual",
          isCorrect: true,
        },
    ...
    },
    {
      ...
      answerOptions: [
        ...
        {
          answerText: "Van a quitar muchos puestos de trabajo",
          isCorrect: false,
        },
  ...
  const handleAnswerClassName = (isCorrect) => {
    if (!isAnswered) {
      ...
      if (isCorrect) {
        return "cursor-default w-100 text-xl bg-green-600 border-[2px] rounded-full text-center border-green-700 flex justify-center items-center p-2 ";
      }
      if (!isCorrect) {
        return "invisible   cursor-default w-100 text-xl bg-red-600 border-[2px] rounded-full text-center border-[#234668] flex justify-center items-center p-2 ";
      }
    }
  };
  ...
  return (
    ...
    className="flex flex-col bg-transparent backdrop-blur-sm h-full border rounded-lg p-8  text-white">
      {showScore ? (
        ...
        <>
          <div className="question-section w-full relative">
            <div className="question-count text-xl mb-8">
              <span className="text-3xl font-bold">{currentQuestion + 1}</span  ><span className="mx-2">/</span>
              {questions.length}
              <span>
                <Image
                  onClick={() => quizClose(false)}
                  className="mt-1 w-[30px] h-[30px] sm:w-[35px] sm:h-[35px] md:w-[25px] md:h-[25px]  float-right hover:cursor-pointer hover:scale-110"
                  src={closeIcon}
                  alt="close button"
                />
              </span>
            ...
            <button
              ...
              className={
                !isAnswered
                  ...
                  : "w-fit text-xl border-[1px] rounded-full bg-[#252d4a] border-[#000] border-opacity-20 text-center flex justify-center items-center self-end p-2 hover:bg-[#5b6898]"
              }
            >
              <Image
